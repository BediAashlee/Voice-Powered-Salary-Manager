//using Context API in the list section (below the create button)
...
const initialState = JSON.parse(localStorage.getItem("transactions")) || [
  // if there is something in the local storage --> then show those, otherwise show these entries
  {
    amount: 500,
    category: "Salary",
    type: "Income",
...
export const Provider = ({ children }) => {
  ...
  const deleteTransaction = (id) => {
    // to delete an entry, we need to know which one are we deleting --> by id
    dispatch({ type: "DELETE_TRANSACTION", payload: id }); // dispatching action --> payload is the additional data we want to pass over with the action
  };
  ...
  const balance = transactions.reduce(
    // calculates final balance after excluding expenses
    (acc, currVal) =>
